...
public class Planet : MonoBehaviour
{
	...
	private void CreateTriangles()
	{
		...
		int[] trianglesZZ = new int[(gridSize * gridSize) * 6];
		int[] trianglesX = new int[(gridSize * gridSize) * 6];
		int[] trianglesXX = new int[(gridSize * gridSize) * 6];
		int[] trianglesY = new int[(gridSize * gridSize) * 6];
		int[] trianglesYY = new int[(gridSize * gridSize) * 6];
		int ring = (gridSize + gridSize) * 2;
		int tZ = 0, tX = 0, tY = 0, tZZ = 0, tXX = 0, tYY = 0, v = 0;
		...
		for (int y = 0; y < gridSize; y++, v++)
		{
			...
			for (int q = 0; q < gridSize; q++, v++)
			{
				tZZ = SetQuad(trianglesZZ, tZZ, v, v + 1, v + ring, v + ring + 1);
			}
			for (int q = 0; q < gridSize - 1; q++, v++)
			{
				tXX = SetQuad(trianglesXX, tXX, v, v + 1, v + ring, v + ring + 1);
			}
			tXX = SetQuad(trianglesXX, tXX, v, v - ring + 1, v + ring, v + 1);
		}

		tY = CreateTopFace(trianglesY, tY, ring);
		tYY = CreateBottomFace(trianglesYY, tYY, ring);
		mesh.subMeshCount = 6;
		mesh.SetTriangles(trianglesZ, 0);
		mesh.SetTriangles(trianglesZZ, 1);
		mesh.SetTriangles(trianglesX, 2);
		mesh.SetTriangles(trianglesXX, 3);
		mesh.SetTriangles(trianglesY, 4);
		mesh.SetTriangles(trianglesYY, 5);
	}
